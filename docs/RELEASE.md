...
正式版本唯一来源是仓库根目录的 `VERSION` 文件。发布标签必须使用 `v<VERSION>`，当前版本使用 `v1.26.817`；构建脚本会拒绝版本格式错误或标签与文件不一致的发布。产物和校验清单文件名也会带上同一个版本标签。
...
```text
dist/datashelf-v1.26.817-linux-amd64
dist/datashelf-v1.26.817-darwin-amd64
dist/datashelf-v1.26.817-darwin-arm64
dist/datashelf-v1.26.817-windows-amd64.exe
dist/SHA256SUMS-v1.26.817
dist/NOTICE
...
构建产物内置同一版本号，可通过 `datashelf-v1.26.817-linux-amd64 -version` 查询。

发布流程只使用 GitHub Actions 内置令牌，不把密钥或配置写入仓库。发布前可在下载端校验：

```bash
sha256sum -c SHA256SUMS-v1.26.817
```
...
install -m 0755 dist/datashelf-v1.26.817-linux-amd64 "$HOME/.local/bin/datashelf-linux-amd64"
...
install -m 0755 dist/datashelf-v1.26.817-darwin-arm64 "$HOME/.local/bin/datashelf-darwin-arm64"
...
Copy-Item .\dist\datashelf-v1.26.817-windows-amd64.exe (Join-Path $install 'datashelf-windows-amd64.exe')
